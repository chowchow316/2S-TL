...
%         ind4=ind(X4T);
         ...
         % to check if data from all labels are there in tst_train_data
          ...
          if length(X2T)==0
               disp(sprintf('label-1 not present in  tst data'));
          end 
%           if length(X3T)==0
...
%          end
      ...
      tst_train_label=[ones(size(tst_train_data_1,1),1); ones(size(tst_train_data_2,1),1)*-1];%2;ones(size(tst_train_data_3,1),1)*3;ones(size(tst_train_data_4,1),1)*4];
    
      tstdata_new=[tstdata_new1; tstdata_new2];%tstdata_new3;tstdata_new4];
      tstlabel_new=[ones(size(tstdata_new1,1),1); ones(size(tstdata_new2,1),1)*-1]; %2;ones(size(tstdata_new3,1),1)*3;ones(size(tstdata_new4,1),1)*4];
